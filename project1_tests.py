...
from project1_events import read_events, get_events_binary_search, sort_events, sort_events_fast

#As for the test file, I only really added tests for get_events_binary_search, as this function was by far the hardest for me to come up with.
...
assert get_doomsday_dotw(2096) == 3
assert get_doomsday_dotw(2121) == 5
assert get_doomsday_dotw(2178) == 6
assert get_doomsday_dotw(2419) == 4

# get_dotw tests
assert get_dotw((4,3,1775)) == 1
...
assert get_events_binary_search((10, 30, 2021)) == []
assert get_events_binary_search((10, 31, 2021)) == ["Halloween"]
...
assert get_events_binary_search((11, 1, 2021)) == ["Dia de los Muertos", "All Saint's Day"] or get_events_binary_search((11, 2, 2021)) == ["All Saint's Day", "Dia de los Muertos"]
...
assert get_events_binary_search((11, 3, 2021)) == []
assert get_events_binary_search((2, 15, 2021)) == []
assert get_events_binary_search((2, 13, 2021)) == []
assert get_events_binary_search((2, 14, 2021)) == ["Valentines Day"]
assert get_events_binary_search((2, 14, 2020)) == ["Valentines Day"]
